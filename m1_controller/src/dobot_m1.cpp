...
DobotM1::DobotM1() : nh_(), pnh_("~")
{
  ...
  DobotM1::IO_cmd_sub_ = nh_.subscribe("io_cmd",1, &DobotM1::IOCmdCallback_,this);
}
...
void DobotM1::IOCmd(uint8_t address, uint8_t level)
{
  uint8_t lv = level;
  //dobot_m1_interface::CheckCommunicationWithException("SetIODO",status);
  dobot_m1_interface::SetIODO(address,lv);
  
}

void TryIOCmd(uint8_t address, uint8_t level)
{
  uint8_t lv = level;

}

void DobotM1::IOCmdCallback_(const std_msgs::Int16 &msg)
{
 uint8_t addr = 18;
 uint8_t lv = 1;
 if (msg.data == 1)
