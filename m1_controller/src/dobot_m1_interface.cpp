...
void SetIODO(uint8_t address, uint8_t level)
{
  dobot_api::IODO iodo;
  iodo.address = address;
  iodo.level = level;
  uint8_t status = dobot_api::SetIODO(&iodo, false, nullptr);
  CheckCommunicationWithException("SetIODO", status);
}

void TrySetIODO(uint8_t address, uint8_t level)
{
  dobot_api::IODO iodo;
  iodo.address = address;
  iodo.level = level;
  uint8_t status = dobot_api::SetIODO(&iodo, false, nullptr);
  CheckCommunication(status);
}
